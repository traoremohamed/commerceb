                        ...
                        <table class="table table-bordered table-hover table-checkable" id="kt_datatable"
                            ...
                            @foreach ($Resultat as $key => $res)
                                <tr>
                                    <td>{{ $res->num_br }}</td>
                                    <td>{{ $res->num_comf }}</td>
                                    <td>{{ $res->date_cre_br }}</td>
                                    <td>{{ $res->lib_fourn }}</td>
                                    <td>{{ $res->lib_agce }}</td>
                                    <td>{{ number_format($res->prix_ttc_br,'0',',','.') }}</td>
                                    <td align="center">
                                        <?php if($res->flag_br == true and $res->annule_br == false){ ?>
                                        <span  class="label label-lg font-weight-bold label-success label-inline"> Validé</span>
                                        <?php  }elseif($res->flag_br == false and $res->annule_br == false){?>
                                        <span class="label label-lg font-weight-bold label-default label-inline">En cours</span>
                                        <?php } elseif ( $res->annule_br == true) { ?>
                                            <span class="label label-lg font-weight-bold label-danger label-inline">Annulé</span>
                                        <?php }  ?>
                                    </td>
                                    <td align="center">
                                        @can($titreRoute.'edit')
                                            <a href="{{ route($titreRoute.'edit',\App\Helpers\Crypt::UrlCrypt($res->num_br)) }}"
