                        ...
                        <table class="table table-bordered table-hover table-checkable" id="kt_datatable"
                            ...
                            @foreach ($Resultat as $key => $res)
                                <tr>
                                    ...
                                    <td>{{ $res->lib_agce  }}</td>
                                    <td>{{ $res->lib_typecli }}</td>
                                    <td>{{'Tel : ' .$res->tel_cli .' / Cel : '. $res->cel_cli .' / Fax : '. $res->fax_cli}}</td>
                                    <td>{{ $res->mail_cli }}</td>
                                    <td>{{ $res->rccm_cli .' / '. $res->cpte_contr_cli }}</td>
                                    <td>{{ $res->adresse_geo_cli }}</td>
                                    <td align="center">
                                        <?php if($res->flag_cli == true){ ?>
                                    ...
                                    <td align="center">
                                        @can($titreRoute.'-edit')
                                            <a href="{{ route($titreRoute.'.edit',$res->num_cli) }}"
                                               class="btn btn-warning btn-xs btn-clean btn-icon"
                                               title="Modifier"> <i class="la la-edit"></i> </a>
                                        @endcan
                                        @can($titreRoute.'-delete')
                                            {!! Form::open(['method' => 'DELETE','route' => [$titreRoute.'.destroy', $res->num_cli],'style'=>'display:inline']) !!}
