                        ...
                        <table class="table table-bordered table-hover table-checkable" id="kt_datatable"
                            ...
                            @foreach ($Resultat as $key => $res)
                                <tr>
                                    <td>{{ $res->num_fourn }}</td>
                                    <td>{{ $res->lib_fourn }}</td>
                                    <td>{{ $res->tel_fourn .' / '. $res->cel_fourn .' / '. $res->fax_fourn}}</td>
                                    <td>{{ $res->mail_fourn }}</td>
                                    <td>{{ $res->rccm_fourn .' / '. $res->cpte_contr_fourn }}</td>
                                    <td>{{ $res->adr_fourn }}</td>
                                    <td align="center">
                                        <?php if($res->flag_fourn == true){ ?>
                                    ...
                                    <td align="center">
                                        @can($titreRoute.'-edit')
                                            <a href="{{ route($titreRoute.'.edit',$res->num_fourn) }}"
                                               class="btn btn-warning btn-xs btn-clean btn-icon"
                                               title="Modifier"> <i class="la la-edit"></i> </a>
                                        @endcan
                                        @can($titreRoute.'-delete')
                                            {!! Form::open(['method' => 'DELETE','route' => [$titreRoute.'.destroy', $res->num_fourn],'style'=>'display:inline']) !!}
